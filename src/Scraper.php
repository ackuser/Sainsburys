...
class Scraper
{
	...
	static function start()
	{
		...
		$params = array('beginIndex' => '0', 'catalogId' => '10137', 'categoryId' => '185749', 'langId' => '44'
		, 'listId' => '', 'listView' => 'true', 'orderBy' => 'FAVOURITES_FIRST', 'pageSize' => '20', 'parent_category_rn' => '12518',
		'promotionId' => '', 'searchTerm' => '', 'storeId' => '10151', 'top_category' => '12518');
		...
		curl_setopt($ch, CURLOPT_ENCODING, 'ISO-8859-1'); // Doesn't seem to work without
		curl_setopt($ch, CURLOPT_USERAGENT, 'Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1; .NET CLR 1.0.3705; .NET CLR 1.1.4322; Media Center PC 4.0)');

		/* Set header took from firebug, otherwise it doesn't receive any html  */
		$headers = array();
		...
		$html = curl_exec($ch);
		if (!$html) //Escape if there is any error
		{
		...
		foreach ($entries as $entry) {
			...
			/* Get title HTML */
			$a = $xpath->query("div//a", $entry); // returns a DOMNodeList
			...
			/* Get linked HTML without assets == HTML content without images, css, js files ... */
			curl_setopt($ch, CURLOPT_URL, $link);//reuse the curl that is open to do more efficient
			$linked_html = curl_exec($ch);
			if (!$linked_html) //Escape if there is any error
			{
				echo "ERROR NUMBER: ".curl_errno($ch);
				echo "ERROR: ".curl_error($ch);
				exit;
			...
			/* Get the description of the product from the linked HTML */
			/* Load linked HTML */
			...
			/* Get Short or Long Description...some products have longer descriptions and they are pàrsed differently */
